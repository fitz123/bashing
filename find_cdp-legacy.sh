...
cat $devip | while read line; do
     ...
     egrep -B 1 "Fas|Gig|Ten" $output > $output2
     ...
     model=`grep "DESCR: " $output | cut -d":" -f3 | tr -d ' ','"' | sed 's/\x0D//g;s/\x08\{3\}//g;s/--More--//g'`

     #Getting the cells
     switches=`cat $output2 | cut -d";" -f1`
     loc_ints=`cat $output2 | cut -d";" -f2`
     sw_mods=`cat $output2 | cut -d";" -f5 | cut -d" " -f1`
     sw_ints=`cat $output2 | cut -d";" -f5 | cut -d" " -f2,3`

     howmany() { echo $#; }
     sw_count=`howmany $switches`
         ...
         while [  $COUNTER -lt $sw_count ]; do
             ...
             switch=`echo -e $switches | cut -d" " -f$s1`
             sw_mod=`echo $sw_mods | cut -d" " -f$s1`
             sw_int=`echo $sw_ints | cut -d" " -f$i1,$i2`
             echo $host$ph$name$ph$model$ph$loc_int$ph$switch$ph$sw_mod$ph$sw_int >> $result
             let i1=i1+2
             let i2=i2+2
